...
```bash
# 2. Click "Export Changes" in the editor — changes persist across page refreshes

# 3. Stop the server with Ctrl+C when done editing

# 4. Apply the changes back to the Python source files
poetry run sprite-editor apply

# 5. Verify the sprites look correct
poetry run sprite-preview

# 6. Run tests to make sure nothing broke
poetry run pytest tests/ -v
```

The apply command backs up originals to `tools/sprite_workspace/backup/` before writing. Only right-facing pig sprites need editing — left-facing variants are auto-generated by mirroring.
