...
A browser-based pixel editor for visually editing all game sprites (pigs, facilities, all zoom levels).

#### End-to-end workflow

```bash
# 1. Export sprite data and open the editor in your browser
poetry run sprite-editor
```

This starts a local server and opens the editor. You'll see all sprites organized by category in the left panel.

**Editing sprites:**
- Select a sprite from the left panel
- Pick a palette key (color) from the right panel
- Click or drag on the pixel grid to paint
- Tools: **Paint** (P), **Erase** (E), **Eyedropper** (I), **Flood Fill** (G)
- **Undo/Redo**: Ctrl+Z / Ctrl+Shift+Z
- Right-click any pixel to pick its color
- Use the zoom slider to resize the grid
- The preview bar at the bottom shows all color palettes simultaneously

**Saving changes:**

Click "Export Changes" in the editor — this automatically applies changes back to the Python source files (only files with actual pixel changes are regenerated). Originals are backed up to `tools/sprite_workspace/backup/`.

```bash
# Stop the server with Ctrl+C when done editing

# Verify the sprites look correct
poetry run sprite-preview

# Run tests to make sure nothing broke
poetry run pytest tests/ -v
```

Only right-facing pig sprites need editing — left-facing variants are auto-generated by mirroring. You can also manually re-apply from `sprite_data.js` with `poetry run sprite-editor apply`.
